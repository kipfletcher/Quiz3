...
def vecSubtract(vector01,vector02):
    ...
    iteration the for loop stores the differences in new_vec. If the two vectors are not compatible
...
def scalarVecMulti(scalar,vector):
    ...
    the product as a new vector. It implements an if and for statement to verify that the scalar and
    vector only contain integers. It uses a for loop to compute each element of the new vector
...
def normalize(vector):
    ...
    return scalarVecMulti(scalar,vector) 
...
#These are test cases for the normalize function with only one in use at a time.
